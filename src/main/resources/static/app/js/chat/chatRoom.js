	...
	fetch(`/resident/chat/room/participant?residentChatRoomId=${residentChatRoomId}`)
		...
		.then(data => {
			...
			data.forEach(p => {
				...
				console.log("로그인 유저 프로필 URL 👉", p.filePathUrl);
				const avatarImg = document.createElement("img");
				avatarImg.className = "avatar rounded-circle";
				avatarImg.width = 24;
				avatarImg.height = 24;
				console.log(`📸 ${p.mbrNnm}의 프로필 이미지 경로:`, p.filePathUrl);

				// 프로필 이미지가 있으면 사용, 없으면 기본 이미지
				avatarImg.src = p.filePathUrl ? p.filePathUrl : "/volt/assets/img/team/beaver.png";
				avatarImg.alt = `${p.mbrNnm}의 프로필`;
				avatarImg.style.marginRight = "8px"; // 이미지와 텍스트 간격

				// 텍스트 노드 생성
				const nameText = document.createTextNode(`${p.unitRoom}호 ${p.mbrNnm}`);
