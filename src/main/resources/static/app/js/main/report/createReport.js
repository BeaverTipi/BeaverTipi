// src/main/resources/static/js/main/report/createReport.js
...
// (참고: 광고 폼의 formatPhoneNumber는 신고 폼에서 필요 없으므로 제거됩니다.)
...
$(document).ready(function() {
    ...
    // 3. 폼 제출 이벤트 (Axios를 사용한 비동기 제출)
    const createReportForm = document.getElementById("createReportForm"); // 폼 ID 변경
    createReportForm.addEventListener("submit", function(event) {
        event.preventDefault(); // 기본 폼 제출 방지

        // 클라이언트 측 유효성 검사 (신고 폼에 맞게 수정)
        ...
        if (!document.getElementById("rptCode").value) {
            alert("신고 유형을 선택해주세요.");
            return;
        }
        if (!document.getElementById("rptTargetId").value) {
            alert("신고 대상 ID 또는 게시글 번호를 입력해주세요.");
            return;
        }
