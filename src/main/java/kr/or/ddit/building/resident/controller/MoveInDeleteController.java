...
public class MoveInDeleteController {

    @Autowired
    private MoveInService moveInService;

    @PostMapping("/delete")
    public ResponseEntity<String> deleteResident(@RequestBody UnitResidentVO vo) {
        log.info("입주민 삭제 요청: {}", vo);
        int result = moveInService.deleteResident(vo);
