...
//    }
    ...
    @GetMapping("/building/product/detail/quick/{lstgId}")
    public String getProductQuickView(@PathVariable("lstgId") String lstgId, Model model) {
        // 1. lstgId로 상세 조회 (Service 호출)
        ListingVO listingVO = service.selectProductById(lstgId);
        // 2. 필요한 데이터 model에 담기
        model.addAttribute("listingVO", listingVO);
        // 3. rentalOwnerProductDetailView.jsp(뷰조각) 리턴
        return "building/product/rentalOwnerProductDetailView"; // .jsp 확장자 없이 경로만!
    }
