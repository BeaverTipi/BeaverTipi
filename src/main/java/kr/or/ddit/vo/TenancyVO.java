...
public class TenancyVO extends MemberVO implements Serializable {
    ...
    @Size(max = 255, groups = {TenancyUpdateGroup.class})
    private String rentalPtyBizRegFilePath;

    @Min(value = 0, groups = {TenancyUpdateGroup.class})
    @Max(value = 999, groups = {TenancyUpdateGroup.class})
    ...
    @Pattern(regexp = "^[YN]$", message = "승인 여부는 'Y' 또는 'N'이어야 합니다.",
             groups = {TenancyUpdateGroup.class})
    private String authApprYn;
    private String authApprDt;
    
    @NotNull(groups = TenancyInsertGroup.class)
