	...
	<div class="card">
		<div class="card-body">
			<form:form modelAttribute="board" method="post">
				...
				<div class="form-control">
				  <label class="label" for="brdCtgryValue">게시판 유형</label>
				  <c:forEach items="${brdCodeList}" var="item">
				    <c:if test="${item.codeValue eq '007'}">
						<form:radiobutton path="brdCtgryValue" value="007" data-sync-code="S0001" />
						  ${item.codeName}
						</c:if>
						<c:if test="${item.codeValue eq '008'}">
						  <form:radiobutton path="brdCtgryValue" value="008" data-sync-code="S0002" />
						  ${item.codeName}
						</c:if>
						<c:if test="${item.codeValue eq '009'}">
						  <form:radiobutton path="brdCtgryValue" value="009" data-sync-code="S0003" />
						  ${item.codeName}
						</c:if>
				  </c:forEach>
				  <form:errors path="brdCtgryValue" cssClass="text-danger" />
				</div>
				...
				<div class="form-detail" id="noticeDetailBox" style="display:none">
				  ...
				  <div>
				  	...
				  	<c:forEach var="item" items="${noticeTypeList}">
				  		 <c:if test="${item.codeValue ne '004'}">
