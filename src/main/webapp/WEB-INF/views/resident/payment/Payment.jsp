...
.ad-box h3, .total-charge-box h3, .combined-paybox h3 {
	margin-top: 0;
	font-size: 16px;
	color: #343a40;
	display: flex;
...
.charge-table td.diff, .charge-table th.diff {
...
select[name="bldgIdParam"] {
	...
	background-image:
		url("data:image/svg+xml;charset=UTF-8,%3Csvg fill='black' height='20' viewBox='0 0 20 20' width='20' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M7 10l5 5 5-5z'/%3E%3C/svg%3E");
...
<body>
	...
	<div class="container">
		...
		<div class="main-grid">
			<div class="notice-grid">
				...
				<c:forEach var="item" items="${chargeBillComparisonList}">
					<c:set var="totalIntegratedAmount"
						value="${totalIntegratedAmount + item.previousAmount}" />
					<c:if test="${item.diffAmount < 0}">
						<c:set var="totalSavedAmount"
							value="${totalSavedAmount + (-item.diffAmount)}" />
					</c:if>
				</c:forEach>
				<c:forEach var="type" items="${energySummary[currentMonth]}">
					<c:set var="energyType" value="${type.key}" />
					<c:if
						test="${energyType eq '전기' || energyType eq '가스' || energyType eq '수도'}">
						<c:set var="row" value="${type.value}" />
						<c:set var="energyChargeSum"
							value="${energyChargeSum + row.chargeAmt}" />
					</c:if>
				</c:forEach>
				<c:set var="totalIntegratedAmount"
					value="${totalIntegratedAmount + energyChargeSum}" />
				<div class="giro-notice-card">
					<h4>
						<fmt:formatDate value="${prevDate}" pattern="yyyy년 MM월" />
						관리비 요약
					</h4>
						...
						<tbody>
							<c:forEach var="item" items="${chargeBillComparisonList}"
								...
								<tr class="${vs.index % 2 == 0 ? 'even' : 'odd'}">
									...
									<td class="amount"><span class="money-compare"> <fmt:formatNumber
												value="${item.previousAmount}" type="currency" /> → <fmt:formatNumber
												value="${item.twoMonthsAgo}" type="currency" />
									</span></td>
									<td class="diff"><c:choose>
											<c:when test="${item.diffAmount > 0}">
												<span class="diff up">▲ <fmt:formatNumber
														value="${item.diffAmount}" />원
												</span>
											</c:when>
											<c:when test="${item.diffAmount < 0}">
												<span class="diff down">▼ <fmt:formatNumber
														value="${-item.diffAmount}" />원
												</span>
											</c:when>
											<c:otherwise>
					...
					<div class="energy-summary">
						<c:forEach var="type" items="${energySummary[currentMonth]}">
							...
							<c:set var="prevRow"
								value="${energySummary[previousMonth][energyType]}" />
							<c:set var="diffUsage"
								value="${currRow.usageQty - prevRow.usageQty}" />
							<c:set var="diffCharge"
								value="${currRow.chargeAmt - prevRow.chargeAmt}" />

							<div class="energy-row">
								<div class="energy-label">🔹 ${energyTypeName}</div>
								<div class="energy-values">
									<div class="value-item">
										<span style="white-space: nowrap;"> 사용량: <fmt:formatNumber
												value="${prevRow.usageQty}" type="number"
												groupingUsed="true" />${unitLabel} → <fmt:formatNumber
												value="${currRow.usageQty}" type="number"
												groupingUsed="true" />${unitLabel}
										</span>
										<c:choose>
											<c:when test="${diffUsage > 0}">
												<span class="diff up">▲ <fmt:formatNumber
														value="${diffUsage}" type="number" groupingUsed="true" />${unitLabel}</span>
											</c:when>
											<c:when test="${diffUsage < 0}">
												<span class="diff down">▼ <fmt:formatNumber
														value="${-diffUsage}" type="number" groupingUsed="true" />${unitLabel}</span>
									...
									<div class="value-item">
										...
										<c:choose>
											<c:when test="${diffCharge > 0}">
												<span class="diff up">▲ <fmt:formatNumber
														value="${diffCharge}" type="currency" /></span>
											</c:when>
											<c:when test="${diffCharge < 0}">
												<span class="diff down">▼ <fmt:formatNumber
														value="${-diffCharge}" type="currency" /></span>
			...
			<div class="ad-sidebar">
				...
				<div class="total-charge-box">
					...
					<p class="total-charge-amount">
						<fmt:formatNumber value="${currentChargeAmount}" type="currency"
							currencySymbol="₩" />
					</p>
					<c:choose>
						<c:when test="${totalSavedAmount > 0}">
							<p style="font-size: 13px; color: #2ecc71; margin-top: 6px;">
								💚 전월 대비 <strong><fmt:formatNumber
										value="${totalSavedAmount}" type="currency" /></strong> 절감되었습니다
							</p>
						</c:when>
						<c:when test="${totalSavedAmount < 0}">
							<p style="font-size: 13px; color: #e74c3c; margin-top: 6px;">
								🔺 전월 대비 <strong><fmt:formatNumber
										value="${-totalSavedAmount}" type="currency" /></strong> 증가하였습니다
							</p>
						</c:when>
						<c:otherwise>
							<p style="font-size: 13px; color: #999; margin-top: 6px;">
								전월과 동일한 금액이 청구되었습니다</p>
						</c:otherwise>
					</c:choose>
				</div>
				<div class="combined-paybox">
					...
					<c:forEach items="${payment}" var="pay">
						<label> <input type="radio" name="payment_method"
							value="${pay.codeName}" checked /> ${pay.codeName}
						</label>
					</c:forEach>
					<!-- ✅ 납부 버튼 추가 -->
					<button type="button" class="pay-button"
						...
						data-charge-month="${currentMonth}">💰 납부하기</button>
				</div>
				<div class="info-box">
					<h3>📘 고객 안내사항</h3>
					<ul style="margin: 0; padding-left: 18px; font-size: 13px;">
						<li>2025년 6월 관리비는 전월 대비 평균 <strong>5.8% 인상</strong>되었습니다.
						</li>
						<li>공용 수도요금은 <strong>배관 정비공사</strong>로 인해 일시적으로 증가했습니다.
						</li>
						<li>엘리베이터 보수공사가 <strong>7월 15일 ~ 18일</strong> 예정되어 있습니다.
						</li>
						<li>관리사무소 운영시간은 <strong>09:00 ~ 18:00</strong>이며, 점심시간은 12:00
							~ 13:00입니다.
						</li>
					</ul>
				</div>
		...
		<div class="double-info-box">
			<div class="info-column">
				<h3>💡 입금 안내</h3>
				<p>
					국민은행 123-456-7890<br />예금주: 동산아파트 관리사무소
				</p>
	...
	<script
		src="${pageContext.request.contextPath}/app/js/building/move-in/residentList.js"></script>
	<script
		src="${pageContext.request.contextPath}/app/js/building/move-in/residentPayment.js"></script>
	<script>
	document.addEventListener("DOMContentLoaded", function () {
		  const unitSelect = document.getElementById("unitSelect");
		  const bldgIdParam = document.getElementById("bldgIdParam");

		  bldgIdParam.addEventListener("change", function () {
		    localStorage.setItem("selectedBuildingId", bldgIdParam.value);
		  });

		  let bldgId = localStorage.getItem("selectedBuildingId");
		  if (!bldgId) {
		    localStorage.setItem("selectedBuildingId", bldgIdParam.value);
		  }
		  ...
		  unitSelect.addEventListener("change", function () {
		    ...
		    const building = localStorage.getItem("selectedBuildingId");
		    if (selectedUnitId) {
